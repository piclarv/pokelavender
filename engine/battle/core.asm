...
; the player is not using Rage and doesn't need to recharge
	...
	ld a, [hl]
	and %01110000 ; check if player is using rollout
	jr nz, .selectEnemyMove
...
HandleRollout:
; get Rollout's power based on rollout counter	
	push hl
...
; EXPLODE_EFFECT and ROLLOUT_EFFECT halve defense.
	cp a, ROLLOUT_EFFECT
	jr z, .halve
	cp a, EXPLODE_EFFECT
	jr nz, .ok
